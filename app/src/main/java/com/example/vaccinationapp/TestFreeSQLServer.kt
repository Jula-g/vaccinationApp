...
object TestFreeSQLServer {
    ...
    fun main(args: Array<String>) {
        ...
        try {
            ...
            // Adding a new record
            addNewRecord(
                conn, "John Deere", "USA", "1990-01-01", "Rossignol", "Rossignol"
            )
            // Deleting a record
            deleteRecord(
                conn, "John Deere"
            ...
            checkAndAddColumn(
                conn, "alpine_skiers", "personal record2"
            )
    ...
    private fun addNewRecord(
        ...
        country: String, date_of_birth: String, skis: String, ski_boots:
        String
    ) {
        try {
            ...
            insertStatement.setDate(
                3,
                java.sql.Date.valueOf(date_of_birth)
            )
    ...
    private fun deleteRecord(conn: Connection, name: String) {
        try {
            val deleteStatement = conn.prepareStatement(
                "DELETE FROM `alpine_skiers` WHERE `name` = ?"
            )
    ...
    private fun retrieveData(
        conn: Connection, selectedCountry:
        String
    ) {
        try {
    ...
    private fun updateData(
        conn: Connection, selectedName:
        String, updateCountry: String
    ) {
        try {
            val updateStatement = conn.prepareStatement(
                "UPDATE `alpine_skiers` SET `country` = ? WHERE `name` = ?"
            )
            ...
            if (rowsAffected > 0) {
                println(
                    "Skier's data has been updated successfully."
                )
            } else {
    ...
    private fun checkAndAddColumn(
        ...
        try {
            ...
            if (resultSet.next()) {
                println(
                    "Column $newColumn already exists in table $table."
                )
            } else {
                ...
                println(
                    "New column $newColumn has been successfully added to table $table."
                )
            }
        } catch (e: SQLException) {
            e.printStackTrace()
    ...
    private fun displayTables(conn: Connection) {
        try {
            println("Tables:")
            val showTablesST = conn.prepareStatement(
                "SHOW TABLES"
            )
            ...
            println("")
            println("*************** skiers ***************")
            val selectAllSt = conn.prepareStatement(
                "SELECT * FROM alpine_skiers;"
            )
